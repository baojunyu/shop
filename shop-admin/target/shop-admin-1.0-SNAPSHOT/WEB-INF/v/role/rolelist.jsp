        ...
        <div class="layui-col-xs3" style="margin-right: 20px">
            <div class="layui-form-item layui-form-text">
                ...
                <div class="layui-input-block">
                    <input type="text" id="no" class="layui-input" placeholder="角色ID">
                </div>
            </div>
        </div>
        <div class="layui-col-xs3" style="margin-right: 20px">
            <div class="layui-form-item layui-form-text">
                ...
                <div class="layui-input-block">
                    <input type="text" id="tel" class="layui-input" placeholder="角色名">
                </div>
            </div>
        </div>
    ...
    layui.use(['table','form','element'],function () {
        ...
        tableIns=table.render({
            ...
            ,cols: [[ //表头
                ...
                ,{field: 'roleUkid', title: 'ID', width:50, sort: true, fixed: 'left'}
                ,{field: 'roleName', title: '角色名', width:120}
                ,{field: 'createTime', title: '创建时间', width:120}
                // ,{field: 'sex', title: '性别', width:45, sort: true}
                // ,{field: 'create_user_id', title: '创建用户ID', width:120}
                ...
                ,{field: 'status', title: '有效/无效', width: 110, sort: true,templet:function (obj) {//obj表示当前的对象
                    var userId=obj.roleUkid;//获取用户id
        ...
        table.on('toolbar(test)', function(obj){
            ...
            switch(obj.event){
                case 'add':
                    layer.msg('添加');
                    window.location.href="${pageContext.request.contextPath}/v/role/roleadd";
                    break;
                case 'update':
                    if(data.length === 0){
                        layer.msg('请选择一行');
        ...
        table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'showpermissions'){
                ...
                window.location.href="${pageContext.request.contextPath}/role/modify?roleUkid="+data.roleUkid;
            ...
            layer.open({
                ...
                yes:function (index) {//第一个按钮触发的事件
                   // alert('取消');
                    ...
                    //发送ajax请求 改变状态
                    $.ajax({
                        url:'${pageContext.request.contextPath}/role/active',
                        data:{"roleUkid":id,"status":status},
                        dataType:'json',
                        type:'post',
                        cache:false,
